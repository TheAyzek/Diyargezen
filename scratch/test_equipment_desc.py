...
def test_cat(category_name):
    c = conn.cursor()
    c.execute("SELECT isim, sistem, kategori, aciklama, sistem_verisi FROM entities WHERE sistem='pathfinder1e' AND kategori IN ('item','equipment') AND isim NOT LIKE '+%' AND isim NOT LIKE '#%' AND isim NOT LIKE '[%' AND isim NOT LIKE '*%'")
    results = []
    for row in c.fetchall():
        ...
        p_prof = (str(sys_obj.get('weaponType') or '') + ' ' + str(sys_obj.get('armorType') or '') + ' ' + str(sys_obj.get('proficiency') or '') + ' ' + str(payload.get('proficiency') or '') + ' ' + desc).lower()
        ...
        results.append(name)
    print(f'Category [{category_name:15s}]: count={len(results):4d} | sample={results[:6]}')
